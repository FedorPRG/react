...
import { deletCardInCart } from "../../reducers/cart_reducer";

export function CartCard({ id, src, name, price, color, size, quantity }) {
  ...
  return (
    <div>
      <div class="cart_box_left_card">
        <img class="cart_box_left_card_img" src={src} alt={src} />
        <div class="cart_box_left_card_info">
          <Link to={`/product/:${id}`} class="cart_box_left_card_info_heading">
            {name}
          </Link>
          <p>
          ...
          <p>Color: {color}</p>
          <p>Size: {size}</p>
          <div class="cart_box_left_card_info_quantity">
            Quantity:
            <p>{quantity}</p>
          </div>
        </div>
        <svg
          onClick={(e) => deleteProductInCart(id)}
          class="cart_box_left_card_closed"
          width="18"
          height="18"
          viewBox="0 0 18 18"
          fill="none"
